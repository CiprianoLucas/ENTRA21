...
biblioteca = {
    1: {"titulo": "O Hobbit", "autor": "J.R.R. Tolkien", "ano": 1937, "genero": "Fantasia", "status": "Disponível",
        "avaliações": [5, 5, 4]},
    2: {"titulo": "1984", "autor": "George Orwell", "ano": 1949, "genero": "Ficção", "status": "Disponível"},
...
def livros_emprestados() -> None:
    ...
    for identificador, livro in biblioteca.items():
        if livro["status"] == "Emprestado":
            livro_emprestado.append((livro['titulo']))

    if livro_emprestado:
        print(f"A listagem de livros emprestados é: {' - '.join(livro_emprestado)}")
    else:
        print("Não há livro emprestados")
